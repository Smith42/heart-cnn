...
def getCNN(classes, finetune=False):
    ...
    if finetune == True:
        ...
        # First layer:
        net = tflearn.layers.conv.conv_3d(net, 32, [10,10,10],  activation="leaky_relu", trainable=False)
        net = tflearn.layers.conv.max_pool_3d(net, [2,2,2], strides=[2,2,2])

        # Second layer:
        net = tflearn.layers.conv.conv_3d(net, 64, [5,5,5],  activation="leaky_relu", trainable=False)
        net = tflearn.layers.conv.max_pool_3d(net, [2,2,2], strides=[2,2,2])

        # Third layer:
        net = tflearn.layers.conv.conv_3d(net, 128, [2,2,2], activation="leaky_relu", trainable=False) # This was added for CNN 2017-07-28
        net = tflearn.layers.conv.max_pool_3d(net, [2,2,2], strides=[2,2,2]) # This was added for CNN 2017-08-24

        #Fourth layer:
        net = tflearn.layers.conv.conv_3d(net, 256, [2,2,2], activation="leaky_relu", trainable=False) # This was added for CNN 2017-08-24
        ...
        net = tflearn.layers.core.fully_connected(net, 2048, activation="leaky_relu", regularizer="L2", weight_decay=0.0001, trainable=False)
        #net = tflearn.layers.core.dropout(net, keep_prob=0.5)

        net = tflearn.layers.core.fully_connected(net, 1024, activation="leaky_relu", regularizer="L2", weight_decay=0.0001, trainable=False)
        #net = tflearn.layers.core.dropout(net, keep_prob=0.5)
        ...
        # Output layer:
        net = tflearn.layers.core.fully_connected(net, classes, activation="softmax")
